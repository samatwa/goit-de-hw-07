...
with DAG(
        ...
        tags=["gregory"]  # Теги для класифікації DAG
) as dag:
    ...
    create_schema = MySqlOperator(
        ...
        sql="""
        CREATE DATABASE IF NOT EXISTS gregory;
        """
    )
    ...
    create_table = MySqlOperator(
        ...
        mysql_conn_id=connection_name,
        sql="""
        CREATE TABLE IF NOT EXISTS gregory.games (
        `edition` text,
    ...
    # Сенсор для порівняння кількості рядків у таблицях `gregory.games` і `olympic_dataset.games`
    check_for_data = SqlSensor(
        task_id='check_if_counts_same',
        conn_id=connection_name,
        sql="""WITH count_in_copy AS (
                select COUNT(*) nrows_copy from gregory.games
                ),
    ...
    # Завдання для оновлення даних у таблиці `gregory.games`
    refresh_data = MySqlOperator(
        task_id='refresh',
        mysql_conn_id=connection_name,
        sql="""
            TRUNCATE gregory.games;  # Очищення таблиці
            INSERT INTO gregory.games SELECT * FROM olympic_dataset.games;  # Вставка даних з іншої таблиці
        """,
    )
