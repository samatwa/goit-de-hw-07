...
connection_name = "goit_mysql_db_oholodetskyi"
...
with DAG(
        ...
        tags=["oholodetskyi"]  # Теги для класифікації DAG
) as dag:
    ...
    create_schema = MySqlOperator(
        ...
        sql="""
        CREATE DATABASE IF NOT EXISTS oholodetskyi;
        """
    )
    ...
    create_table = MySqlOperator(
        ...
        mysql_conn_id=connection_name,
        sql="""
        CREATE TABLE IF NOT EXISTS oholodetskyi.games (
        `edition` text,
    ...
    # Сенсор для порівняння кількості рядків у таблицях `oholodetskyi.games` і `olympic_dataset.games`
    check_for_data = SqlSensor(
        task_id='check_if_counts_same',
        conn_id=connection_name,
        sql="""WITH count_in_copy AS (
                select COUNT(*) nrows_copy from oholodetskyi.games
                ),
    ...
    # Завдання для оновлення даних у таблиці `oholodetskyi.games`
    refresh_data = MySqlOperator(
        task_id='refresh',
        mysql_conn_id=connection_name,
        sql="""
            TRUNCATE oholodetskyi.games;  # Очищення таблиці
            INSERT INTO oholodetskyi.games SELECT * FROM olympic_dataset.games;  # Вставка даних з іншої таблиці
        """,
    )
