...
with DAG(
    ...
    create_schema = MySqlOperator(
        ...
        sql="""
        CREATE DATABASE IF NOT EXISTS staskut;
        """
    )
    ...
    create_table = MySqlOperator(
        ...
        mysql_conn_id=connection_name,
        sql="""
        CREATE TABLE IF NOT EXISTS staskut.games (
        `edition` text,
    ...
    check_for_data = SqlSensor(
        ...
        sql="""WITH count_in_copy AS (
                select COUNT(*) nrows_copy from staskut.games
                ),
    ...
    refresh_data = MySqlOperator(
        ...
        mysql_conn_id=connection_name,
        sql="""
            TRUNCATE staskut.games;  # Очищення таблиці
            INSERT INTO staskut.games SELECT * FROM olympic_dataset.games;  # Вставка даних з іншої таблиці
        """,
    )
