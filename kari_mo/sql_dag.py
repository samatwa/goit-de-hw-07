...
with DAG(
        ...
        tags=["kari"]  # Теги для класифікації DAG
) as dag:
    ...
    create_table = MySqlOperator(
        ...
        mysql_conn_id=connection_name,
        sql="""
        CREATE TABLE IF NOT EXISTS kari.games (
        `edition` text,
    ...
    # Сенсор для порівняння кількості рядків у таблицях `kari.games` і `olympic_dataset.games`
    check_for_data = SqlSensor(
        task_id='check_if_counts_same',
        conn_id=connection_name,
        sql="""WITH count_in_copy AS (
                select COUNT(*) nrows_copy from kari.games
                ),
    ...
    # Завдання для оновлення даних у таблиці `kari.games`
    refresh_data = MySqlOperator(
        task_id='refresh',
        mysql_conn_id=connection_name,
        sql="""
            TRUNCATE kari.games;  # Очищення таблиці
            INSERT INTO kari.games SELECT * FROM olympic_dataset.games;  # Вставка даних з іншої таблиці
        """,
    )
