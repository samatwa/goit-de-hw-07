...
connection_name = "goit_mysql_db_vy"
...
with DAG(
    ...
    create_schema = MySqlOperator(
        ...
        sql="""
        CREATE DATABASE IF NOT EXISTS veronika_y;
        """
    )
    ...
    create_table = MySqlOperator(
        ...
        mysql_conn_id=connection_name,
        sql="""
        CREATE TABLE IF NOT EXISTS veronika_y.games (
        `edition` text,
    ...
    # Сенсор для перевірки рівності кількості рядків у таблиці `veronika_y.games` і `olympic_dataset.games`
    check_for_data = SqlSensor(
        task_id='check_if_counts_same',
        conn_id=connection_name,
        sql="""WITH count_in_copy AS (
                select COUNT(*) nrows_copy from veronika_y.games
                ),
    ...
    # Завдання для оновлення даних у таблиці `veronika_y.games`
    refresh_data = MySqlOperator(
        task_id='refresh',
        mysql_conn_id=connection_name,
        sql="""
            TRUNCATE veronika_y.games;  # Очищення таблиці
            INSERT INTO veronika_y.games SELECT * FROM olympic_dataset.games;  # Вставка даних з іншої таблиці
        """,
    )
